...
def write(filename, data, channel_names=None, precision=FLOAT, compression=PIZ_COMPRESSION):
    ...
    def make_ndims_3(matrix):
        if matrix.ndim > 3 or matrix.ndim < 2:
            raise Exception(
                "Invalid number of dimensions for the `matrix` argument.")
        elif matrix.ndim == 2:
    ...
    def get_channel_names(channel_names, depth):
        if channel_names:
            if depth is not len(channel_names):
                raise Exception("The provided channel names have the wrong length (%d vs %d)." % (
                    len(channel_names), depth))
            return channel_names
        elif depth in _default_channel_names:
            return _default_channel_names[depth]
        else:
            raise Exception(
                "There are no suitable default channel names for data of depth %d" % depth)
    ...
    if isinstance(data, dict):
        ...
        if not isinstance(precision, dict):
            ...
            precisions = {group: precision.get(
                group, FLOAT) for group in data.keys()}

        # Prepare channel names
        if channel_names is None:
            channel_names = {}
        channel_names = {group: get_channel_names(channel_names.get(
            group), matrix.shape[2]) for group, matrix in data.items()}
        ...
        for group, matrix in data.items():
            ...
            if len(names) != depth:
                raise Exception(
                    "Depth does not match the number of channel names for channel '%s'" % group)
            for i, c in enumerate(names):
                ...
                channel_data[channel_name] = matrix[:, :, i].astype(
                    NP_PRECISION[str(precisions[group])]).tostring()
    ...
    #
        ...
        header = OpenEXR.Header(width, height)
        header['compression'] = compression
        header['channels'] = {c: Imath.Channel(
            precision) for c in channel_names}
        out = OpenEXR.OutputFile(filename, header)
        out.writePixels({c: data[:, :, i].astype(
            NP_PRECISION[str(precision)]).tostring() for i, c in enumerate(channel_names)})

    else:
        raise Exception(
            "Invalid precision for the `data` argument. Supported are NumPy arrays and dictionaries.")
...
class InputFile(object):
    ...
    def __init__(self, input_file, filename=None):
        ...
        self.channels = sorted(
            header['channels'].keys(), key=_channel_sort_key)
        self.depth = len(self.channels)
        self.precisions = [c.type for c in header['channels'].values()]
        self.channel_precision = {c: v.type for c,
                                  v in header['channels'].items()}
    ...
    def describe_channels(self):
        ...
        for group in sorted(list(self.root_channels)):
            if group != 'default':
                ...
                print("%-20s%s" %
                      (group, ",".join([c[len(group)+1:] for c in channels])))
    ...
    def get(self, group='default', precision=FLOAT):
        ...
        matrix = np.zeros((self.height, self.width, len(
            channels)), dtype=NP_PRECISION[str(precision)])
    ...
    def get_dict(self, groups=[], precision={}):
        ...
        for group in groups:
            ...
            if group in precision:
                ...
                p = FLOAT
            matrix = np.zeros((self.height, self.width, len(
                group_chans)), dtype=NP_PRECISION[str(p)])
        ...
        for i, item in enumerate(todo):
            precision = NP_PRECISION[str(
                self.channel_precision[todo[i]['channel']])]
